<x-app-layout>
    <x-slot name="pageTitle">Add City</x-slot>
    ...
    <div class="max-w-lg">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100 dark:border-slate-700">
                <h2 class="text-lg font-bold text-gray-800 dark:text-slate-100">Add New City</h2>
                <p class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">Cities appear in travel request origin/destination dropdowns</p>
            </div>

            <form action="{{ route('settings.cities.store') }}" method="POST" class="p-6 space-y-5">
                @csrf
